...
def App_v02():
    ...
    if error_stop: 
        ...
        SummaryActivity_DF = Activity.labelStand_v3(SummaryActivity_DF)
        # st.dataframe(SummaryActivity_DF)
    ...
    #######################################################################################################################################
        ...
        # with st.expander('SubActivity Summary'):
            ...
            Pie_Layout[0].plotly_chart(figPie_Activity, use_container_width=True)
            Pie_Layout[1].markdown("#### Sub-Activity Pie Chart by " + WellName_Select + " " + SectionSelect+ " in Hole Section")
            Pie_Layout[1].plotly_chart(figPie_SubActivity, use_container_width=True)    
        ...
        with st.expander("Time vs Depth"):
            ...
            st.plotly_chart(figTimeDepth, use_container_width=True) 
            # figTimeDepth.show()
        ...
        with st.expander("Meterage Drilling per Date"):
            ...
            #         xaxis_label='depth'
            # )
            MeterageDrilling_Layout = st.columns(1)
            ...
            MeterageDrilling_Layout[0].markdown("#### Drilling Meterage " + WellName_Select + " " + SectionSelect+ " in Hole Section")
            MeterageDrilling_Layout[0].plotly_chart(figMeterageDrillingDate_v2, use_container_width=True)
            # fig.show()
        ...
        with st.expander("ROP On Bottom and Stand"):
            if OffsetWellPlot_Logic:
                ...
                Chart_05_Layout[0].plotly_chart(figOBH_ROP, use_container_width=True)
                Chart_05_Layout[1].markdown("#### Average On Bottom and Stand by Well" + " " + SectionSelect+ " Hole Section")
                Chart_05_Layout[1].plotly_chart(figOBH_ROP_OffsetWell, use_container_width=True)
        ...
        with st.expander('Stand Time Breakdown'):
            if OffsetWellPlot_Logic:
                ...
                Chart_06_Layout[0].plotly_chart(figStandTimeBreakdown, use_container_width=True)

                Chart_06_Layout[1].markdown("#### Stand Break Down by Well" + " " + SectionSelect+ " Hole Section")
                Chart_06_Layout[1].plotly_chart(figOffsetWell_StandTimeBreakdown, use_container_width=True)
        ...
        ConnectionTime_DF = SummaryActivity_DF.copy()
        ConnectionTime_DF = ConnectionTime_DF[ConnectionTime_DF['ACTIVITY'].isin(["DRILLING FORMATION", 
                                                                                'DRILL OUT CEMENT',
                                                                                "CIRCULATE HOLE CLEANING"
                                        ])]

        ConnectionTime_DF = ConnectionTime_DF.groupby(['CONNECTION-ACTIVITY']).agg({
                    'Start Time':'first', 
                    'Duration (Minutes)':'sum',
                    }
                ).reset_index().sort_values(by='Start Time')
        # st.dataframe(ConnectionTime_DF)
        ConnectionTime_DF[['CONNECTION-ACTIVITY', 'Connection-Number']] = ConnectionTime_DF['CONNECTION-ACTIVITY'].str.split('--', 1, expand=True)
        # st.dataframe(ConnectionTime_DF)
        ConnectionTime_DF = ConnectionTime_DF.join(ConnectionTime_DF.groupby(['Connection-Number']).agg({'Start Time':'first'}), on='Connection-Number', rsuffix='_used')
        # st.dataframe(ConnectionTime_DF)
        ConnectionTime_DF = ConnectionTime_DF[ConnectionTime_DF['CONNECTION-ACTIVITY'].isin([
                                                                                            'Pre-Connection',
                                                                                            'Connection',
                                                                                            'Post-Connection'
                                                                                        ])]
        # st.text(ConnectionTime_DF.dtypes)
        with st.expander('Connection Time'):
            figSConnectionTime = px.bar(ConnectionTime_DF, x='Start Time_used', y=['Duration (Minutes)'],color='CONNECTION-ACTIVITY', width=500, height=500)
            # figStandTimeBreakdown = px.bar(SummaryActivity_DF.dropna(subset=['Stand Group_Pred', "Duration(minutes)"]), x='Stand Group_Pred', y="Duration(minutes)", color="LABEL_SubActivity", width=1000, height=1000)
            # figSConnectionTime.update_xaxes(tickformat="%b\n%Y")
            figSConnectionTime.update_layout(
                            # title="Title",
                            xaxis=dict(
                                title="Date",
                            ),
            ...
            # figSConnectionTime.update_xaxes(tickformat="%b\n%Y")
            st.markdown("#### Connection Time " + WellName_Select + " " + SectionSelect+ " in Hole Section")
            st.plotly_chart(figSConnectionTime, use_container_width=True)
        #     figStandTimeBreakdown = px.bar(ConnectionTime_DF, x='Start Time_used', y=['Duration (Minutes)'],color='SUB-ACTIVITY', width=500, height=500)
        #     # figStandTimeBreakdown = px.bar(SummaryActivity_DF.dropna(subset=['Stand Group_Pred', "Duration(minutes)"]), x='Stand Group_Pred', y="Duration(minutes)", color="LABEL_SubActivity", width=1000, height=1000)
        #     figStandTimeBreakdown.update_xaxes(type='category')
        #     figStandTimeBreakdown.update_layout(
        #                     # title="Title",
