...
from PDU_Func import Authentification, IO_Data, ActivityMapping
...
def App():
    ...
    RigActivityCols[0].markdown("##### Rig Status")
    ...
    RigActivityCols[1].markdown("##### Well Parameters")
    ...
    if not st.session_state['IsFormSubmit']:
        ...
        RTSensor_df = cache_DomeGetRealtimeSensorData(WellInfoDict,UserDateRange, hours=0.5, show_progress=True, container=st.container())
        ActivityMapping
        # InputActivity_DB = ActivityMapping.translateRigActivity2Activity(RigActivity_DF)
        RTSensor_df = ActivityMapping.addRigActivityLabel (
                            RTSensor_df, 
                            ActivityMapping.translateRigActivity2Activity(RigActivity_DF)
                            )
        RTSensor_df = ActivityMapping.addSectionParams (
                            RTSensor_df, 
                            SectionParams_DF
                            )
