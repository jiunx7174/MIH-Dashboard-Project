...
def getStandLabel(ActSum_df, DrillActivityList='default',stand_num=None):
    ...
    for k,ActSum_df_Temp in ActSum_df.groupby((ActSum_df['LABEL_Activity'].shift() != ActSum_df ['LABEL_Activity']).cumsum()):
        if ActSum_df_Temp['LABEL_Activity'].tolist()[0] in (DrillActivityList):
            ...
            for idx_conn_start, idx_conn_end in zip(idx_conn_start_list, idx_conn_end_list):
                    idx = idx_conn_end
                    idx_start = idx_conn_start + 1
                    idx_end = idx_conn_end - 1
...
def predictSubActivityLabel(RTSensor_df, TripActivityList='default', DrillActivityList='default', OverrideActivityList='default', RunCasingActivityList='default'):
    ...
    list_columns = ["dt", 'date', 'time', "bitdepth", "md", "blockpos", "rop", "hklda", "woba", "torqa", "speeddown", "speedup",
